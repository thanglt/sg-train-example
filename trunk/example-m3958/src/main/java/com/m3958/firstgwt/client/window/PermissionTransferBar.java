...
public class PermissionTransferBar extends VLayout{
	...
	public void setup(String rid,ListGrid rightGrid){
		this.rid = rid;
	...
	private TransferImgButton initRightArrowButton() {
		...
		arrowButton.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent event) {
            	...
            	RPCManager.sendRequest(getRpcRequest(rid,rs, "false"),new MyRpcCallback(){
					@Override
	...
	private TransferImgButton initLeftArrowButton() {
		...
		arrowButton.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent event) {
            	...
            	RPCManager.sendRequest(getRpcRequest(rid,rs, "true"),new MyRpcCallback(){
					@Override
					public void afterSuccess(RPCResponse response, Object rawData, RPCRequest request, JSONValue data) {
						leftGrid.invalidateCache();
					}});
	...
	private RPCRequest getRpcRequest(String rid,Record[] roles,String b){
    	String rids = "";
    	...
    	m.put(SmartParameterName.RELATION_MODEL_NAME, PermissionDataSource.className);
    	m.put(SmartParameterName.EXTRAPARAS, rids);
    	m.put(SmartParameterName.MODEL_ID, rid);
    	m.put(SmartParameterName.MODEL_NAME, RoleDataSource.className);
