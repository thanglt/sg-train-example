...
public class PayApplyButtonToolBar extends BaseButtonToolStript {

	private ListGrid listGrid;

	public PayApplyButtonToolBar(ListGrid listGrid) {
		super("account.applyManager.payApplyManager");
	...
	protected void showWindow(String buttonName, ToolStripButton button) {
		...
		if ("add".equalsIgnoreCase(buttonName)) {
			useWindow = new PayApplyAddWindow("新建付款申请书", false,srcRect, listGrid, "showwindow/finance_add_01.png");
			...
			if (listGrid.getSelection().length == 1) {
				...
				for(Record rd:rdd){
				 ...
				 useWindow = new PayApplyModifyWindow("修改付款申请书", false,srcRect, listGrid, "showwindow/finance_modify_01.png");
			} else {
				SC.warn("请选择一条记录进行修改！");
				return;
			}
			...
			if (listGrid.getSelection().length == 1) {
				...
				for(Record rd:rdd){
				 ...
				 useWindow = new CheckPayApplyWindow("审核付款申请书", false,srcRect, listGrid, "showwindow/finance_modify_01.png");
			} else {
				SC.warn("请选择一条记录进行审核！");
				return;
			}
