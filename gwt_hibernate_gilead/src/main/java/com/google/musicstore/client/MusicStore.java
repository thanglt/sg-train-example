        ...
        for (Account account : result) {
          ...
          if (records != null) {
            for (Record record : records) {
              ...
              if (first) {
                accountRecords.setText(index, 0, String
  ...
  private void constructRecordsToAccountPanel(
    ...
    addRecord.addClickHandler(new ClickHandler() {
      public void onClick(ClickEvent event) {
        ...
        musicStoreService.saveRecordToAccount(account, record,
    ...
    musicStoreService.getRecords(new AsyncCallback<List<Record>>() {
      ...
      public void onSuccess(List<Record> result) {
        ...
        for (Record record : result) {
          recordTitles.addItem(record.getTitle(), String
  ...
  private void loadAccounts(final HorizontalPanel addRecordsToAccountPanel,
    ...
    musicStoreService.getAccounts(new AsyncCallback<List<Account>>() {
      ...
      public void onSuccess(List<Account> result) {
        ...
        accountIds.clear();
        for (Account account : result) {
          accountIds.addItem(String.valueOf(account.getId()), String
              .valueOf(account.getId()));
        }
      }
    });
