...
public class PurchaseInvoiceButtonToolBar extends BaseButtonToolStript {
	...
	public PurchaseInvoiceButtonToolBar(
			ListGrid listGrid) {
		// TODO Auto-generated constructor stub
		super("account.invoiceManager.payInvoiceManager");
	...
	protected void showWindow(String buttonName, ToolStripButton button) {
		...
		if ("add".equalsIgnoreCase(buttonName)) {
			useWindow = new PurchaseInvoiceAddWindow("新建付款发票", false,srcRect, listGrid, "showwindow/finance_add_01.png");
			...
			if (listGrid.getSelection().length == 1) {
				...
				for(Record rd:rdd){
				 ...
				 useWindow = new PurchaseInvoiceModifyWindow("修改付款发票", false,srcRect, listGrid, "showwindow/finance_modify_01.png");
			} else {
				SC.warn("请选择一条记录进行修改！");
				return;
			}
			...
			if (listGrid.getSelection().length == 1) {
				...
				useWindow = new CheckPurchaseInvoiceWindow("审核付款发票", false, srcRect, listGrid, "showwindow/finance_modify_01.png");
			} else {
				SC.warn("请选择一条记录进行审核！");
				return;
			}
