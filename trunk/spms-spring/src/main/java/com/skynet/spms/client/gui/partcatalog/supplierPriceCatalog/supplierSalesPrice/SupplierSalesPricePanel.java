...
import com.skynet.spms.client.gui.partcatalog.salesCatalog.salesPrice.AlternateSupplyLocationListGrid;
import com.skynet.spms.client.gui.partcatalog.salesCatalog.salesPrice.OtherChargeListGrid;
import com.skynet.spms.client.gui.partcatalog.salesCatalog.salesPrice.PriceBreakListGrid;
...
public class SupplierSalesPricePanel extends VLayout{
	...
	public SupplierSalesPricePanel() {
		DataSourceTool dataSourceTool = new DataSourceTool();
		dataSourceTool.onCreateDataSource("partCatalog.supplierPrice", "supplierSalesPrice_dataSource", new PostDataSourceInit() {
			@Override
	...
	public void initPanel(DataSource dataSource,DataInfo dataInfo) {
		...
		//特殊包装价格
		 ...
		 texSpecialPackPrice.setRequired(true);
		...
		//一类容器价格单位
		SelectItem selCocpCurrencyCode=(SelectItem)dataInfo.getFieldInfoByName("cocpCurrencyCode").generFormField();
		selCocpCurrencyCode.setDefaultToFirstOption(true);
		selCocpCurrencyCode.setWidth(80);
		itemList.add(selCocpCurrencyCode);
		...
		//供应商件号数量
		TextItem txtQuantity = new TextItem("partCatalogQuantity");
		txtQuantity.setValidators(ValidateUtils.isFloatValidator());
		txtQuantity.setValidateOnChange(true);
		txtQuantity.setWidth(100);
		itemList.add(txtQuantity);
		//供应商件号数量单位
		SelectItem selPcqUnitCode=(SelectItem)dataInfo.getFieldInfoByName("pcqUnitCode").generFormField();
		selPcqUnitCode.setDefaultToFirstOption(true);
		selPcqUnitCode.setWidth(80);
		itemList.add(selPcqUnitCode);
		...
		formSaveButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				if(form.getItem("id").getValue() == null){
					Window.alert(ConstantsUtil.partCatalogConstants.alertForSelectSupplierPriceIndex());
					return;
				}
	...
	public void fetchData(final String spSalesPriceId){	
		Criteria criteria = new Criteria("id",spSalesPriceId);
        form.fetchData(criteria,new DSCallback() {	
			...
			public void execute(DSResponse response, Object rawData, DSRequest request) {
				...
				form.getItem("partCatalogQuantity").setValue(record.getAttribute("partCatalogQuantity"));
				form.getItem("pcqUnitCode").setValue(record.getAttribute("pcqUnitCode"));
	        }
		});
        
        Criteria criteria2 = new Criteria("priceId",spSalesPriceId);
