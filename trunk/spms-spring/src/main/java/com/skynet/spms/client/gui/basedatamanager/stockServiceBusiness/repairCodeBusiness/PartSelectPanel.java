...
import com.skynet.spms.client.gui.basedatamanager.stockServiceBusiness.partsInventory.partsInventoryRecord.PartsInventoryRecordListgrid;
...
public class PartSelectPanel extends VLayout{

	private PartsInventoryRecordListgrid originListGrid;
	private PartsInventoryRecordListgrid targetListGrid;
	private List<Record> recordList;
	private HLayout buttonPanel;
	
	public PartSelectPanel(){
		this(null);
	}
	public PartSelectPanel(final String repairCodeId){
		recordList = new ArrayList<Record>();
		setWidth100();
		
		DataSourceTool dataSourceTool = new DataSourceTool();
		String partModuleName = "stockServiceBusiness.partsInventory.partsInventoryRecord";
		String partDsName = "partsInventoryRecord_dataSource";
		
		//需补码的航材库存件
		targetListGrid = new PartsInventoryRecordListgrid();
		targetListGrid.setHeight("40%");
		dataSourceTool.onCreateDataSource(partModuleName, partDsName, new PostDataSourceInit() {	
			@Override
		...
		//所有航材库存件
		originListGrid = new PartsInventoryRecordListgrid();
