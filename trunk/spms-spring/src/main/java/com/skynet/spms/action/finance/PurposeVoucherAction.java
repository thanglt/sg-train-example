...
@Component
public class PurposeVoucherAction implements DataSourceAction<PurposeVoucher> {

	private Logger log = LoggerFactory.getLogger(PurposeVoucherAction.class);

	@Autowired
	private PurposeVoucherManager purposeVoucherManager;
	...
	public String[] getBindDsName() {
		...
		return new String[] { "finance_purposeVoucher_dataSource","finance_payVoucher_dataSource" };
	}
	...
	public void insert(PurposeVoucher item) {
		...
		for(Object obj : item.getClassificMapItems()){
			Map map = (Map)obj;
			String subject = (String)map.get("subject");
			Subjects sub = classificationItemManager.querySubjectsById(subject);
			map.put("subject", sub);
		}
		if (item.getClassificationItem() != null) {
		...
		item.setCreateCertificateUser(user);
