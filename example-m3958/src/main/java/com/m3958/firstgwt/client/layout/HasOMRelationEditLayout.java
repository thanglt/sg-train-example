...
public abstract class HasOMRelationEditLayout extends MyVlayout implements Iview{
	...
	public HasOMRelationEditLayout(){
		setMargin(3);
		setBorder("1px solid grey");
		setMembersMargin(5);
	}
	
	protected abstract String getRelationPropertyName();
	
	protected DSRequest createDsRequest(){
		DSRequest dr = new DSRequest();
		Map<String, String> m = new HashMap<String, String>();
		if(getRelationPropertyName() == null){
			m.put(SmartParameterName.RELATION_MODEL_ID, relationModelId);
		}else{
			m.put(getRelationPropertyName(), relationModelId);
		}
		dr.setParams(m);
		return dr;
	}
