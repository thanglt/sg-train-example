 ...
 * the License.
 */
package com.m3958.firstgwt.client.view.gwt;
...
public class RichTextToolbar extends Composite {
  ...
  private class EventHandler implements ClickHandler, ChangeHandler,
    ...
    public void onKeyUp(KeyUpEvent event) {
      int i = event.getNativeKeyCode();
      boolean iscc = event.isControlKeyDown();
      Widget sender = (Widget) event.getSource();
      if (sender == richText) {
        // We use the RichTextArea's onKeyUp event to update the toolbar status.
        // This will catch any cases where the user moves the cursur using the
...
//  private RichTextArea.ExtendedFormatter extended;
  ...
  public void insertImg(String imgUrl){
	  basic.insertImage(imgUrl);
  }
  
  public void insertImg(AssetJso aj,String sizeHint){
	  if("原图".equals(sizeHint))sizeHint="";
	  String html;
	  if(sizeHint == null || sizeHint.isEmpty()){
		  html = "<img src=\"/asset/" + aj.getId() +  aj.getExtension()  + "\" title= \"" + aj.getOriginName() + "\" alt = \"" + aj.getOriginName() +  "\"/>";
	  }else{
		  html = "<img src=\"/asset/" + aj.getId() + "." + sizeHint + aj.getExtension()  + "\" title= \"" + aj.getOriginName() + "\" alt = \"" + aj.getOriginName() +  "\"/>";
	  }
	  basic.insertHTML(html);
  }
  
  public void insertLink(AssetJso aj){
	  String html = "<a href=\"/asset/" + aj.getId() +  aj.getExtension()  + "\">" + aj.getOriginName() + "</a>";
	  basic.insertHTML(html);
  }
}
