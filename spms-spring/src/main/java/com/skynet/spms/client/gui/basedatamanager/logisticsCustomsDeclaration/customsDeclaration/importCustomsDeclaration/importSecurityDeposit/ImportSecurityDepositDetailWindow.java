 ...
 *
 */
public class ImportSecurityDepositDetailWindow extends Window {
	 ...
	 * @param iconUrl
	 */
	public ImportSecurityDepositDetailWindow(String windowTitle,
			boolean isMax,
		...
		// 进口报关保证金ID
		final HiddenItem txtImportSecurityDepositID = new HiddenItem("id");
		// 报关ID
		...
		// 到期日期
		final CustomDateItem txtExpireDates = new CustomDateItem("expireDates", "到期日期");
		...
		// 办保原因
		final TextAreaItem txtReason = new TextAreaItem("reason", "办保原因");
		txtReason.setColSpan(5);
		txtReason.setHeight(35);
		txtReason.setWidth(520);
		// 担保金额
		...
		// 备件项
		final ImportSecurityDepositItemsListgrid importSecurityDepositSparepartsListgrid =
			new ImportSecurityDepositItemsListgrid();
		importSecurityDepositSparepartsListgrid.setWidth(610);
		importSecurityDepositSparepartsListgrid.setHeight(210);
		importSecurityDepositSparepartsListgrid.setMargin(5);
		importSecurityDepositSparepartsListgrid.setAutoSaveEdits(false);
		importSecurityDepositSparepartsListgrid.setSelectionType(SelectionStyle.SINGLE);
		...
		dataSourceTool.onCreateDataSource(modeName, dsName,
				new PostDataSourceInit() {
					public void doPostOper(DataSource dataSource,
							DataInfo dataInfo) {
						importSecurityDepositSparepartsListgrid.setDataSource(dataSource);
						
						importSecurityDepositSparepartsListgrid.drawImportSecurityDepositItemsListgrid();
						if (isView == true) {
							importSecurityDepositSparepartsListgrid.setCanEdit(false);	
						} else {
							importSecurityDepositSparepartsListgrid.setCanEdit(true);
						}
						importSecurityDepositSparepartsListgrid.setEditEvent(ListGridEditEvent.CLICK);
						if (txtCustomsID.getValue() != null)
						{
							importSecurityDepositSparepartsListgrid.fetchData(new Criteria("customsID", "" + txtCustomsID.getValue().toString()));
						}
					}
				});
		...
		headForm.setFields(txtImportSecurityDepositID
				,txtCustomsID
				,txtOrderID
				,txtContractNumber
		...
		secondForm.setFields(txtImportSecurityDepositID
				,txtCustomsID
				,txtOrderID
		...
		saveButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				// 设置明细数据
				ListGridRecord[] detailRecord = new ListGridRecord[importSecurityDepositSparepartsListgrid.getTotalRows()];
				for (int i = 0; i < importSecurityDepositSparepartsListgrid.getTotalRows(); i++)
				{
					detailRecord[i] = (ListGridRecord)importSecurityDepositSparepartsListgrid.getEditedRecord(i);
				}
		...
		layout.addMember(importSecurityDepositSparepartsListgrid);
