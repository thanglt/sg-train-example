...
public class EnterpriseInfoAddWindow extends BaseWindow {

	public EnterpriseInfoAddWindow(String windowTitle, boolean isMax,
	...
	protected Canvas getLeftLayout(final Rectangle srcRect, ListGrid listGrid) {
		...
		addCloseClickHandler(new CloseClickHandler() {
			public void onCloseClick(CloseClientEvent event) {
				ShowWindowTools.showCloseWindow(srcRect, EnterpriseInfoAddWindow.this, -1);
			}
		});
		...
		//公司标志//加载企业的图标；
		TextAreaItem txtLogo = new TextAreaItem("logoURL");
		...
		if(enterpriseInfoTreeGrid.getSelection().length == 1){
			String parentId = enterpriseInfoTreeGrid.getSelectedRecord().getAttribute("id");
			String companyName = enterpriseInfoTreeGrid.getSelectedRecord().getAttribute("abbreviation");
			sltParentEnterprise.setValue(parentId);
			txtParentCompany.setValue(companyName);
			txtParentCompany.setDisabled(false);
		}
	    ...
	    sltCountryCode.setHint(requiredHint);
	    sltCountryCode.setRequired(true);
	    sltCountryCode.setDefaultToFirstOption(true);
	    ...
	    /*final SelectItem sltCAGECode = new SelectItem("cageCodeId","企业代码");
	    DataSourceTool dataSourceTool = new DataSourceTool();
	    dataSourceTool.onCreateDataSource("user.enterprise", "CAGECode_dataSource",new PostDataSourceInit() {
			@Override
			public void doPostOper(DataSource dataSource, DataInfo dataInfo) {
				sltCAGECode.setOptionDataSource(dataSource);
				sltCAGECode.setDisplayField("code");
				sltCAGECode.setDefaultToFirstOption(true);
			}
		});
	    FormItemIcon addCAGECodeIcon = new FormItemIcon();
	    sltCAGECode.setIcons(addCAGECodeIcon);
	    sltCAGECode.setIconPrompt("新增");
	    sltCAGECode.addIconClickHandler(new IconClickHandler() {
			@Override
			public void onIconClick(IconClickEvent event) {
				AddCodeWindow addCAGECodeWindow = new AddCodeWindow(
						"新增企业代码", 
						"企业代码",
						"code",
						sltCAGECode.getOptionDataSource());
				addCAGECodeWindow.show();
			}
		});
	    itemList.add(sltCAGECode);*/


	    //是否主营企业
		SelectItem sltIsMain = new SelectItem("mainEnterprise");
		sltIsMain.setValueMap(valueMap);
		sltIsMain.setDefaultToFirstOption(true);
	    ...
	    sltIsRunning.setDefaultToFirstOption(true);
		...
		String prop = ClassifyHelper.getPropByDataSource(enterpriseInfoTreeGrid.getDataSource().getID());
		if(prop != null){
		...
		if(ClassifyHelper.isCustomer(enterpriseInfoTreeGrid.getDataSource().getID())){
			...
			sltCategoryCode.setDefaultToFirstOption(true);
		...
		if(ClassifyHelper.canAppoint(enterpriseInfoTreeGrid.getDataSource().getID())){
			...
			sltIsAppoint.setDefaultToFirstOption(true);
