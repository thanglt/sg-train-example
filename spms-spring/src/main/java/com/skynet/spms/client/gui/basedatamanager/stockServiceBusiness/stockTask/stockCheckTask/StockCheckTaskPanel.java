...
public class StockCheckTaskPanel extends ShowcasePanel {

	private static final String DESCRIPTION = "盘点任务跟踪信息维护页面";

	public static class Factory implements PanelFactory {

		private String DESCRIPTION = "盘点任务跟踪模块";
		private String id;
		
		public Canvas create() {
			StockCheckTaskPanel panel = new StockCheckTaskPanel();
	...
	public Canvas getViewPanel() {
		...
		stockTaskListgrid.addFetchDataHandler(new FetchDataHandler() {
			...
			public void onFilterData(FetchDataEvent event) {
				...
				criteria.addCriteria("taskType", "ST");
		...
		// 获取数据源
		String modeName = "stockServiceBusiness.stockTask.stockCheckTask";
		...
		// 明细列表Grid
		final StockCheckTaskItemListgrid shelveTaskDetailManageListgrid = new StockCheckTaskItemListgrid();
		shelveTaskDetailManageListgrid.setHeight("100%");
		// 获取数据源
		String detailmodeName = "stockServiceBusiness.stockTask.stockCheckTask";
		String detaildsName = "stockCheckTaskItem_dataSource";
		...
		detaildataSourceTool.onCreateDataSource(detailmodeName, detaildsName,
				new PostDataSourceInit() {
					...
					public void doPostOper(DataSource dataSource,
						...
						shelveTaskDetailManageListgrid.drawStockCheckTaskItemListgrid();
					}
				});
		
		// 共用按钮面板
		StockCheckTaskButtonPanel shelveTaskButtonPanel = new StockCheckTaskButtonPanel(stockTaskListgrid, shelveTaskDetailManageListgrid);
		...
		// 主列表面板
		SectionStackSection headSection = new SectionStackSection("盘点任务信息");
		headSection.setItems(stockTaskListgrid);
		headSection.setExpanded(true);

		// 明细列表面板
		SectionStackSection detailSection = new SectionStackSection("盘点任务明细信息");
