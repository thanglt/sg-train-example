...
@Singleton
public class ShareToGroupWindow extends Window{

	private String objectIdentity;
	private String modelName;
	private ListGrid sharedGroupGrid;
	...
	@Inject
	public ShareToGroupWindow(GroupDataSource gds){
		this.gds = gds;
		setWidth(600);
		setHeight(450);
		setIsModal(false);
	...
	private void fetchData() {
		...
		c.addCriteria(SmartParameterName.SUB_OPERATION_TYPE,SmartSubOperationName.FETCH_SHARED_GROUPS.toString());
		sharedGroupGrid.filterData(c);
	}
	...
	private RPCRequest getRpcRequest(boolean isAsign){
    	...
    	m.put(SmartParameterName.SUB_OPERATION_TYPE, SmartSubOperationName.SHARE_TO_GROUP.toString());
    	m.put(SmartParameterName.MODEL_ID, objectIdentity + "");
    	m.put(SmartParameterName.MODEL_NAME, modelName);
    	m.put(SmartParameterName.EXTRAPARAS, shareForm.getValueAsString("gid") + "," + shareForm.getValueAsString("orn"));
    	...
    	if(rollOverRecord != null){
    		...
    		m.put("gid", rollOverRecord.getAttributeAsString("id"));
    	}
...
//							if(b){
								...
								sharedGroupGrid.invalidateCache();
//							}else{
...
//							}
	...
	private Canvas initShareForm() {
		...
		TextItem userNameItem = new TextItem("gid","組id或名称");
	...
	private Canvas initSharedUserGrid() {
	    ...
	    sharedGroupGrid.setShowRollOverCanvas(true);
		sharedGroupGrid.setDataSource(gds);
		sharedGroupGrid.setAutoFetchData(false);
		ListGridField idField = new ListGridField("id");
		idField.setHidden(true);
		ListGridField mainNameField = new ListGridField("mainName","名稱");
		ListGridField subNameField = new ListGridField("subName","英文名稱");
		...
		sharedGroupGrid.setFields(idField,mainNameField,subNameField,roleNameField,roleIdField);
		return sharedGroupGrid;
	}
}
