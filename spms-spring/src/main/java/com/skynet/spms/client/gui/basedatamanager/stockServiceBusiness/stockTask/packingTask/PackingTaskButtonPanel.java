...
public class PackingTaskButtonPanel extends BaseButtonToolStript {

	private StockTaskListgrid stockTaskListgrid;
	private PackingTaskItemListgrid pickingTaskItemListgrid;
	
	public PackingTaskButtonPanel(StockTaskListgrid stockTaskListgrid,
			PackingTaskItemListgrid pickingTaskItemListgrid) {
		super("stockServiceBusiness.stockTask.packingTask");
	...
	protected void showWindow(String buttonName, ToolStripButton objButton) {
		...
		if ("picking".equalsIgnoreCase(buttonName)) {
			...
			if (pickingTaskItemListgrid.getSelection().length >= 1) {
				...
				record.setAttribute("pickingTaskDetailIDs", barcodeTagUUIDs);
